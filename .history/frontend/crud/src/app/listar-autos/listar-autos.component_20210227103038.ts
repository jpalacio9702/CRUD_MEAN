...
export class ListarAutosComponent implements OnInit {
  ...
  filtrar( filterValue: string ){
    ...
    if (!this.autoMarca.length){
      ...
      this.lista = this.listaIni.filter((list: { marca: string | any[]; }) => list.marca.includes(filterValueLower));
      // this.lista = this.listaIni.filter((lista) => lista.name.includes(filterValueLower));
    }
  }
