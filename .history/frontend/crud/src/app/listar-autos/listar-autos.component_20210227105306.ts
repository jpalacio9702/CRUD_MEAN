...
export class ListarAutosComponent implements OnInit {
  ...
  filtrar(){
    if (!this.autoMarca.length){
      ...
      this.lista = this.listaIni.filter((list: { marca: string | modelo: string }) => list.marca.toLowerCase().includes(this.autoMarca) || list.modelo.toLowerCase().includes(this.autoMarca) );
    }
  }
